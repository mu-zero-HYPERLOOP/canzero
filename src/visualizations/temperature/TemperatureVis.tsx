...
function TemperatureVis() {
  ...
  const sac_ebox = useObjectEntryValue("input_board", "sac_ebox_temperature");
  const power_ebox = useObjectEntryValue("input_board", "power_ebox_temperature");
  const supercaps = useObjectEntryValue("input_board", "supercap_temperature");
  ...
  useEffect(() => {
    ...
    if (sac_ebox === undefined) {
      svg.style.setProperty("--ebox2_temperature", "#000");
    } else if ((sac_ebox as number) > 80) {
      svg.style.setProperty("--ebox2_temperature", "#f00");
    } else if (sac_ebox as number > 70) {
      svg.style.setProperty("--ebox2_temperature", "#ff0");
    } else if (sac_ebox as number > 10) {
      svg.style.setProperty("--ebox2_temperature", "#0f0");
    } else {
  ...
  useEffect(() => {
    ...
    if (power_ebox === undefined) {
      svg.style.setProperty("--ebox1_temperature", "#000");
    } else if ((power_ebox as number) > 80) {
      svg.style.setProperty("--ebox1_temperature", "#f00");
    } else if (power_ebox as number > 70) {
      svg.style.setProperty("--ebox1_temperature", "#ff0");
    } else if (power_ebox as number > 10) {
      svg.style.setProperty("--ebox1_temperature", "#0f0");
    } else {
  ...
  useEffect(() =>{
    const svg = document.getElementById("temperature_vis")!;
    if (driver === undefined) {
      svg.style.setProperty("--motor_driver_temperature", "#000");
    } else if ((driver as number) > 80) {
      svg.style.setProperty("--motor_driver_temperature", "#f00");
    } else if (driver as number > 70) {
      svg.style.setProperty("--motor_driver_temperature", "#ff0");
    } else if (driver as number > 10) {
      svg.style.setProperty("--motor_driver_temperature", "#0f0");
    } else {
  ...
  useEffect(() =>{
    const svg = document.getElementById("temperature_vis")!;
    if (driver === undefined) {
      svg.style.setProperty("--dslim_starboard_temperature", "#000");
      svg.style.setProperty("--dslim_port_temperature", "#000");
    } else if ((driver as number) > 80) {
      svg.style.setProperty("--dslim_starboard_temperature", "#f00");
      svg.style.setProperty("--dslim_port_temperature", "#f00");
    } else if (driver as number > 70) {
      svg.style.setProperty("--dslim_starboard_temperature", "#ff0");
      svg.style.setProperty("--dslim_port_temperature", "#ff0");
    } else if (driver as number > 10) {
      svg.style.setProperty("--dslim_starboard_temperature", "#0f0");
      svg.style.setProperty("--dslim_port_temperature", "#0f0");
    } else {
      svg.style.setProperty("--dslim_starboard_temperature", "#00f");
    ...
    <Paper sx={{
      ...
      <Stack paddingTop="3vh">
        ...
        >
          <g id="frame">
            <g id="mgu2">
              <Tooltip title={`${(gfl as number)?.toFixed(2)}C`}>
			...
			c2.88,0,5.24,2.36,5.24,5.24v7.07C438.4,124.47,436.05,126.83,433.16,126.83z"/>
              ...
              <Tooltip title={`${(gfr as number)?.toFixed(2)}C`}>
			...
			c2.88,0,5.24-2.36,5.24-5.24v-7.07C438.8,597.38,436.44,595.03,433.56,595.03z"/>
              ...
              <Tooltip title={`${(gbl as number)?.toFixed(2)}C`}>
			...
			c2.88,0,5.24,2.36,5.24,5.24v7.07C1025.18,124.19,1022.82,126.55,1019.94,126.55z"/>
              ...
              <Tooltip title={`${(gbr as number)?.toFixed(2)}C`}>
		...
		C1123.41,436.92,1118.01,442.32,1111.41,442.32z"/>
          ...
          <Tooltip title={`${(sac_ebox as number)?.toFixed(2)}C`}>
            <g id="ebox1">
		...
		C536.83,432.12,531.43,437.52,524.83,437.52z"/>
          ...
          <Tooltip title={`${(power_ebox as number)?.toFixed(2)}C`}>
            <g id="ebox2">
		...
		C436.9,430.88,431.5,436.28,424.9,436.28z"/>
          ...
          <Tooltip title={`${(driver as number)?.toFixed(2)}C`}>
		...
		c6.6,0,12,5.4,12,12v141.05C911.15,435.67,905.75,441.07,899.15,441.07z"/>
          ...
          <Tooltip title="Cooling Reservoir: Not connected">
            <g id="reservoir">
              <circle className="cooling_reservoir" cx="1069.66" cy="323.45" r="50.92" />
            </g>
          </Tooltip>
          <Tooltip title={`${(supercaps as number)?.toFixed(2)}C`}>
            <g id="supercaps">
		...
		c7.68,0,13.96,6.28,13.96,13.96v98.4C1008.97,415.18,1002.69,421.46,995.01,421.46z"/>
              <path className="scaps" d="M950.99,336.01h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C962.99,330.61,957.59,336.01,950.99,336.01z"/>
              <path className="scaps" d="M991.73,336.29h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C1003.73,330.89,998.33,336.29,991.73,336.29z"/>
              <path className="scaps" d="M950.99,375.93h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C962.99,370.53,957.59,375.93,950.99,375.93z"/>
              <path className="scaps" d="M991.73,376.21h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C1003.73,370.81,998.33,376.21,991.73,376.21z"/>
              <path className="scaps" d="M950.99,416.47h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C962.99,411.07,957.59,416.47,950.99,416.47z"/>
              <path className="scaps" d="M991.73,416.76h-12.38c-6.6,0-12-5.4-12-12v-12.38c0-6.6,5.4-12,12-12h12.38c6.6,0,12,5.4,12,12v12.38
		C1003.73,411.36,998.33,416.76,991.73,416.76z"/>
          ...
          <Tooltip title={`${(dslim as number)?.toFixed(2)}`}>
            <g id="DSLIM">
		...
		h-47.56c-2.32,0-4.54-0.98-6.09-2.71l-2.98-3.29h-16.41l-2.33,2.92c-1.56,1.95-3.92,3.08-6.41,3.08h-40.48v39.32h279.39V309.91z"/>
          ...
          <g id="mlus">
            ...
            <Tooltip title={`${(l1l as number)?.toFixed(2)}C`}>
              <g>
		...
		c2.43,0,4.42,1.99,4.42,4.42v11.77C431.01,218.44,429.02,220.43,426.59,220.43z"/>
            ...
            <Tooltip title={`${(l1r as number)?.toFixed(2)}C`}>
              <g>
		...
		c2.43,0,4.42,1.99,4.42,4.42v11.77C726.37,218.89,724.38,220.88,721.94,220.88z"/>
            ...
            <Tooltip title={`${(l2l as number)?.toFixed(2)}C`}>
              <g>
		...
		c2.43,0,4.42,1.99,4.42,4.42v11.77C1024.69,218.88,1022.7,220.88,1020.27,220.88z"/>
            ...
            <Tooltip title={`${(l3l as number)?.toFixed(2)}C`}>
              <g>
		...
		c2.43,0,4.42,1.99,4.42,4.42v11.77C431.44,562.63,429.44,564.62,427.01,564.62z"/>
            ...
            <Tooltip title={`${(l2r as number)?.toFixed(2)}C`}>
              <g>
		...
		c2.43,0,4.42,1.99,4.42,4.42v11.77C726.79,563.08,724.8,565.07,722.37,565.07z"/>
            ...
            <Tooltip title={`${(l3r as number)?.toFixed(2)}C`}>
              <g>
