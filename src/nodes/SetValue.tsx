...
function sendInput(node: string, objectEntry: string, num: number | string, type: ObjectEntryType) {
...
function checkAndSendInput(node: string, objectEntry: string, val: string, type: ObjectEntryType, setError: {
    ...
    if (isInt(type)) {
        if (!val.includes(".")) {
            ...
            if (!isNaN(num)) {
                sendInput(node, objectEntry, num, type)
                return;
            }
        }
    } else if (isUint(type)) {
        ...
        sendInput(node, objectEntry, val, type)
        return;
    }
    setError(true)
}
